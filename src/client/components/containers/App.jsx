...
class App extends Component {
  ...
  componentDidMount() {
    // This file will listen on all of these channels(selectAll, deselectAll, etc) for any communication from the swell.js file(aka the main process)
    ...
    window.onerror = (error, url, line) => {
      ...
      if (errorCount % 2 !== 0 && error !== "Uncaught Error: Protocol error") {
        let answer = dialog.showMessageBoxSync({
          title: `Application Error (${errorCount})`,
          message: `(${errorCount}) An error has occurred, you can click Continue to keep working or click Refresh Page to refresh`,
          buttons: ["Refresh Page", "Continue"],
        });
        if (answer === 0) {
